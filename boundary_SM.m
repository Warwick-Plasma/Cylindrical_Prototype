...
for imode=0:Nmode
    ...
    for j=4:nr+5
        ...
        %   THIS
        % 2. MISSING -DT*JTM(1,J,IMODE+1)/(1+DT/DX)
        % 3. WE DO NOT CONSIDER J=3, BRM ON THE R=0 AXIS
        Brm(1,j,imode+1) = Alpha_Xmin * Etm(1,j,imode+1) ...
            + Beta_Xmin * Brm(2,j,imode+1) ...
            - Gamma_Xmin * laser_P_xmin ...
            + Delta_Xmin * (Bxm(1,j+1,imode+1) - Bxm(1,j,imode+1));
    end
    ...
    for j=4:nr+6
        ...
        % x=Xmin
        Btm(1,j,imode+1) = -Alpha_Xmin * Erm(1,j,imode+1) ...
            + Beta_Xmin * Btm(2,j,imode+1) ...
            + Gamma_Xmin * laser_S_xmin ...
            + Epsilon_Xmin * imode / rj * Bxm(1,j,imode+1);
    end
    
    % Redo condition on axis for Bt because it was modified
    % Btm(1,3,2): Use d(B_perp)/dtheta = 0 on r=0
    % Btm(1,3,imode+1): Use Btm(3)=-2iBrm(3) - Btm(4) where Brm(3)=0
    if imode == 1 
        Btm(1,3,2) = -2 * imagi * Brm(1,3,2) - Btm(1,4,2);
    else
        Btm(1,3,imode+1) = -Btm(1,4,imode+1);
    end
    ...
    for j=4:nr+5
        ...
        % 2. MISSING +DT*JTM(1,J,IMODE+1)/(1+DT/DX)
        % 3. WE DO NOT CONSIDER J=3, BRM ON THE R=0 AXIS
        Brm(nx+6,j,imode+1) = -Alpha_Xmax * Etm(nx+5,j,imode+1) ...
            + Beta_Xmax * Brm(nx+5,j,imode+1) ...
            + Gamma_Xmax * laser_P_xmax ...
            + Delta_Xmax * (Bxm(nx+5,j+1,imode+1) - Bxm(nx+5,j,imode+1));
    end
    ...
    for j=4:nr+6
        ...
        Btm(nx+6,j,imode+1) = Alpha_Xmax * Erm(nx+5,j,imode+1) ...
            + Beta_Xmax  * Btm(nx+5,j,imode+1) ...
            - Gamma_Xmax * laser_S_xmax ...
            + Epsilon_Xmax * imode / rj * Bxm(nx+5,j,imode+1);
    end

    % Redo condition on axis for Bt because it was modified
    % Btm(nx+6,3,2): Use d(B_perp)/dtheta = 0 on r=0
    % Btm(nx+6,3,imode+1): Use Btm(3)=-2iBrm(3) - Btm(4) where Brm(3)=0
    if imode == 1
        Btm(nx+6,3,2)= -2 * imagi * Brm(nx+6,3,2) - Btm(nx+6,4,2);
    else
        Btm(nx+6,3,imode+1) = -Btm(nx+6,4,imode+1);
    end
end